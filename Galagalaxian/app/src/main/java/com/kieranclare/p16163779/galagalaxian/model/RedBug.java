...
public class RedBug extends EnemyShip implements Visitable{
     ...
     */
    public RedBug(float widthIn, float heightIn, float xIn, float yIn, float speed, Map<String, Bitmap> sprites, SoundEffects soundEffectsObject) {
        super(widthIn, heightIn, xIn, yIn, Color.RED, speed);
        eType = enemyType.RED;
        this.spriteIdle = new Sprite(sprites.get("red"), getWidth(), getHeight(), x, y);
    ...
    public void generateUniqueAttackPattern(Point screenSize, GridPattern gridPattern, PointF t){
        ...
        temp.lineTo(t.x + getWidth(), screenSize.y + screenSize.y*0.1f);
