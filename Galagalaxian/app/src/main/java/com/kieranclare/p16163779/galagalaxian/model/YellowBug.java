...
public class YellowBug extends EnemyShip implements Visitable{
     ...
     */
    YellowBug(float widthIn, float heightIn, float xIn, float yIn, float speed, Map<String, Bitmap> sprites, SoundEffects soundEffectsObject) {
        super(widthIn, heightIn, xIn, yIn, Color.YELLOW, speed);
        eType = enemyType.YELLOW;
        this.spriteIdle = new Sprite(sprites.get("yellow"), getWidth(), getHeight(), x, y);
        this.spriteIdle.addFrameFromMaster(0, 0, (int)(spriteIdle.masterBitmap.getWidth()*0.5f), spriteIdle.masterBitmap.getHeight());
    ...
    public void generateUniqueAttackPattern(Point screenSize, GridPattern gridPattern, PointF t){
        ...
        if(t.x >= screenSize.x*0.5f) {
            temp.lineTo(t.x + screenSize.x*0.3f, screenSize.y + screenSize.y*0.3f);
            temp.arcTo( t.x - screenSize.x*0.3f, screenSize.y - screenSize.y*0.3f, t.x + screenSize.x*0.3f, screenSize.y + screenSize.y*0.3f,
                    0, 180, false);
            temp.arcTo( t.x - screenSize.x*0.3f, screenSize.y - screenSize.y*0.3f, t.x + screenSize.x*0.3f, screenSize.y + screenSize.y*0.3f,
                    180, 180, false);
        }else{
        ...
        temp.lineTo(gridPattern.getPosition(gridx, gridy, tempInt)[0], gridPattern.getPosition(gridx, gridy, tempInt)[1]);
        addPath(temp);
    }
    ...
    public YellowBug getShip(Visitor visitor) {
